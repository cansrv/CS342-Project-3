...
void dma_free(void *p)
{
    ...
    for (int i = (int)index; i < (int)pow(2, segment_size - 3); i += 2)
    {
        ...
        unsigned int b0 = (unsigned int)(current_char >> (7 - in_char_index)) % 2;
        unsigned int b1 = (unsigned int)(current_char >> (6 - in_char_index)) % 2;
        int internal_break = 0;
        // printf("i: %d, current_char: %hhu, b0: %d, b1: %hhu , area_found:%d\n", i, current_char, b0, b1, area_found);
        if (!b0 && b1)
        {
            area_found += 2;
            for (int j = 2 + i; j < (int)pow(2, segment_size - 3) - i; j += 2)
            {

                current_char = *((char *)starting_pointer + j / 8);
                in_char_index = j % 8;
                b0 = (unsigned int)(current_char >> (7 - in_char_index)) % 2;
                b1 = (unsigned int)(current_char >> (6 - in_char_index)) % 2;
                // printf("i: %d, current_char: %hhu, b0: %d, b1: %hhu , area_found:%d\n", j, current_char, b0, b1, area_found);
    ...
    for (int i = index; i < end_index + 2; i++)
    {
        insert_1((char *)starting_pointer, i / 8, i % 8);
    }
}
...
void dma_print_blocks()
{
    for (int i = 0; i < (int)pow(2, segment_size - 3); i += 2)
    {
        ...
        unsigned int b0 = (unsigned int)(current_char >> (7 - in_char_index)) % 2;
        unsigned int b1 = (unsigned int)(current_char >> (6 - in_char_index)) % 2;
        if (!b0 && b1) // If the block is allocated
        {
            int size = 2;
            while (1)
            {
                i += 2;
                current_char = *((char *)starting_pointer + i / 8);
                int in_char_index = i % 8;
                unsigned int b0 = (unsigned int)(current_char >> (7 - in_char_index)) % 2;
                unsigned int b1 = (unsigned int)(current_char >> (6 - in_char_index)) % 2;
                if (b0 || b1)
                {
                    printf("A, 0x%lx, 0x%x %d\n", (unsigned long)starting_pointer + (i - size) * 8 + 256, size * 8, size * 8);
                    i -= 2;
                    break;
                }

                if (i >= (int)pow(2, segment_size - 3))
                {
        ...
        else if (b0 && b1) // If the block is free
        {
            int size = 2;
            while (1)
            {
